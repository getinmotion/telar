...
-- Migración idempotente - versión corregida
-- ============================================

-- 1. Crear enum para roles de entorno (solo si no existe)
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'environment_role') THEN
...
-- 4. Agregar columna environment a todas las tablas relevantes (solo si no existe)
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'agent_tasks' AND column_name = 'environment'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'agent_conversations' AND column_name = 'environment'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'agent_messages' AND column_name = 'environment'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'agent_deliverables' AND column_name = 'environment'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'user_master_context' AND column_name = 'environment'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'task_generation_history' AND column_name = 'environment'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'agent_usage_metrics' AND column_name = 'environment'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'task_steps' AND column_name = 'environment'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'step_validations' AND column_name = 'environment'
  ) THEN
...
-- 9. Crear índices para performance (solo si no existen)
...
DROP POLICY IF EXISTS "Cualquiera puede ver entornos activos" ON public.environments;
CREATE POLICY "Cualquiera puede ver entornos activos"
ON public.environments FOR SELECT
USING (is_active = true);

DROP POLICY IF EXISTS "Solo admins pueden modificar entornos" ON public.environments;
...
DROP POLICY IF EXISTS "Usuarios ven su propio acceso" ON public.user_environment_access;
...
DROP POLICY IF EXISTS "Solo admins pueden gestionar accesos" ON public.user_environment_access;
...
DROP POLICY IF EXISTS "Users can view tasks in their environment" ON public.agent_tasks;
...
DROP POLICY IF EXISTS "Users can create tasks in their environment" ON public.agent_tasks;
...
DROP POLICY IF EXISTS "Users can update tasks in their environment" ON public.agent_tasks;
...
DROP POLICY IF EXISTS "Users can delete tasks in their environment" ON public.agent_tasks;
...
DROP POLICY IF EXISTS "Users can view conversations in their environment" ON public.agent_conversations;
...
DROP POLICY IF EXISTS "Users can create conversations in their environment" ON public.agent_conversations;
...
DROP POLICY IF EXISTS "Users can update conversations in their environment" ON public.agent_conversations;
...
DROP POLICY IF EXISTS "Users can delete conversations in their environment" ON public.agent_conversations;
...
DROP POLICY IF EXISTS "Users can view messages in their environment" ON public.agent_messages;
...
DROP POLICY IF EXISTS "Users can create messages in their environment" ON public.agent_messages;
...
DROP POLICY IF EXISTS "Users can view deliverables in their environment" ON public.agent_deliverables;
...
DROP POLICY IF EXISTS "Users can create deliverables in their environment" ON public.agent_deliverables;
...
DROP POLICY IF EXISTS "Users can update deliverables in their environment" ON public.agent_deliverables;
...
DROP POLICY IF EXISTS "Users can delete deliverables in their environment" ON public.agent_deliverables;
...
DROP POLICY IF EXISTS "Users can view master context in their environment" ON public.user_master_context;
...
DROP POLICY IF EXISTS "Users can insert master context in their environment" ON public.user_master_context;
...
DROP POLICY IF EXISTS "Users can update master context in their environment" ON public.user_master_context;
...
DROP POLICY IF EXISTS "Solo admins pueden ver snapshots" ON public.environment_snapshots;
CREATE POLICY "Solo admins pueden ver snapshots"
ON public.environment_snapshots FOR SELECT
USING (public.check_admin_access());

DROP POLICY IF EXISTS "Solo admins pueden crear snapshots" ON public.environment_snapshots;
...
DROP POLICY IF EXISTS "Usuarios ven logs de sus entornos" ON public.environment_activity_log;
...
DROP POLICY IF EXISTS "Sistema puede insertar logs" ON public.environment_activity_log;
...
DROP TRIGGER IF EXISTS update_environments_timestamp ON public.environments;
