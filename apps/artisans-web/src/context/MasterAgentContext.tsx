...
export const MasterAgentProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const { user } = useAuth();
  const { getCurrentUserCached, getUserProfileCached, invalidateCache } = useDataCache();
  const [masterState, setMasterState] = useState<MasterAgentState>(initialState);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [lastSyncTime, setLastSyncTime] = useState<number>(0);
  const [eventsInProgress, setEventsInProgress] = useState<Set<string>>(new Set());
  ...
  const refreshModule = useCallback(async (moduleName: keyof MasterAgentState) => {
    setIsLoading(true);
    setError(null);
      ...
      switch (moduleName) {
        ...
        case 'marca': {
          try {
            ...
            const contextData = await getUserMasterContextByUserId(authUser.id);

            const brandEval = (contextData?.businessContext as any)?.brand_evaluation;

            // ✅ BUSCAR LOGO TAMBIÉN EN artisan_shops
            const shopData = await getArtisanShopByUserId(authUser.id).catch(() => null);

            const logoUrl = profile?.avatarUrl || brandEval?.logo_url || shopData?.logoUrl || null;

            if (profile || brandEval || shopData?.logo_url) {
              const hasLogo = !!logoUrl;
              const hasColors = (brandEval?.colors?.length || 0) > 0;
              const hasClaim = !!(brandEval?.claim || profile?.businessDescription);
              ...
              setMasterState(prev => ({
                ...
                marca: {
                  ...
                  claim: brandEval?.claim || profile?.businessDescription || '',
                  score: brandEval?.score || score,
                  updated_at: brandEval?.evaluation_date || profile?.updatedAt,
                },
              }));
            } else {
        ...
        case 'tienda': {
          const shop = await getArtisanShopByUserId(authUser.id).catch((error) => {
        ...
        case 'inventario': {
          ...
          const shop = await getArtisanShopByUserId(authUser.id).catch((error) => {
            console.error('[MasterAgent] Error loading shop for inventory:', error);
            return null;
          });
          ...
          const lowStock = productsArray.filter((p: any) => (p.inventory || 0) < 5);
          const sinPrecio = productsArray.filter((p: any) => !p.price || p.price === 0);
          const stockTotal = productsArray.reduce((sum: number, p: any) => sum + (p.inventory || 0), 0);
        ...
        case 'pricing': {
          ...
          if (materialsError) {
            console.error('[MasterAgent] Error loading materials:', materialsError);
          }
        ...
        case 'growth': {
          ...
          try {
            ...
            const tasksWithSteps = Array.from(tasksMap.values()).sort((a, b) =>
            ...
            console.error('[MasterAgent] Error loading user_master_context:', error);
          }
          break;
        }
      }
  ...
  const getModuleState = useCallback(<K extends keyof MasterAgentState>(
    moduleName: K
  ): MasterAgentState[K] => {
    return masterState[moduleName];
  }, [masterState]);

  const updateTaskStep = useCallback(async (taskId: string, stepId: string, completed: boolean) => {
    try {
  ...
  }, [user?.id, syncAll]); // ✅ Incluir syncAll en dependencias
  ...
  useEffect(() => {
    const unsubscribers = [
      ...
      EventBus.subscribe('business.profile.updated', async ({ userId }) => {
        ...
        if (userId === user?.id) {
          invalidateCache(['userProfile']);
          setEventsInProgress(prev => new Set(prev).add('business.profile.updated'));
          try {
      ...
      EventBus.subscribe('brand.updated', async (data: any) => {
        ...
        try {
          ...
          const existingShop = await getArtisanShopByUserId(user.id).catch(() => null);
          ...
          if (existingShop) {
            ...
            try {
              await updateArtisanShop(existingShop.id, {
                ...
                description: data.businessDescription || existingShop.description
              });
