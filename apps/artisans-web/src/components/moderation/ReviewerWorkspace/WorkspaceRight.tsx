        ...
        >
          <div
            style={{
              ...
              width: `${pct}%`,
              backgroundColor:
                pct < 40 ? "#ef4444" : pct < 75 ? "#f59e0b" : color,
            }}
          />
        </div>
      </div>
...
function scoreInterpretation(score: number): {
  ...
  if (score >= 75)
    return {
      ...
      color: "#34d399",
      text: "Este producto cumple con los criterios de calidad del marketplace TELAR.",
    };
  if (score >= 50)
    return {
      bg: "rgba(251,191,36,0.12)",
      color: "#fbbf24",
      text: "El producto necesita mejoras antes de destacarse en el marketplace.",
    };
  return {
    bg: "rgba(248,113,113,0.12)",
    color: "#f87171",
    text: "Calidad insuficiente — requiere revisión antes de aprobarse.",
  };
}

export const WorkspaceRight: React.FC<WorkspaceRightProps> = ({
  product,
  history,
}) => {
  ...
  const analysisData: ModerationAnalyzeRequest = useMemo(
    () => ({
      ...
      image_urls: Array.isArray(product.images)
        ? product.images.filter(Boolean)
        : [],
      price: product.price,
  ...
  return (
    <ScrollArea className="h-full">
      <div style={{ display: "flex", flexDirection: "column" }}>
        ...
        <div
          style={{
            background: "linear-gradient(135deg, #151b2d 0%, #1e2842 100%)",
            padding: 16,
          }}
        >
          <p
        ...
        {history.length > 0 && (
          <div style={{ padding: 16 }}>
            ...
            >
              <History
                style={{ width: 13, height: 13, color: "rgba(84,67,62,0.35)" }}
              />
              <p
                style={{
                  fontFamily: SANS,
