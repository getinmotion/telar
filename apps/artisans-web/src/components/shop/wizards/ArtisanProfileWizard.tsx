...
const STEPS = [
  {
    ...
    icon: "person",
    title: "Tu identidad",
    subtitle: "Cada taller tiene una voz única. Empecemos por la tuya.",
  },
  {
    n: 2,
    icon: "history_edu",
    title: "Tu historia y tradición",
    subtitle:
      "Cuéntanos cómo aprendiste tu oficio y su relación con tu territorio.",
  },
  {
    n: 3,
    icon: "storefront",
  ...
  },
  {
    ...
    icon: "visibility",
    title: "Vista previa",
    subtitle: "Revisa cómo se presentará tu historia antes de publicar.",
  },
];

const AI_PANEL: Record<
  number,
  { cards: Array<{ label: string; text: string }>; next: string }
> = {
  1: {
    cards: [
      {
        label: "Lectura de identidad",
        text: "Esperando nombre, foto o descripción para iniciar el análisis.",
      },
      {
        label: "Presentación pública",
        text: "TELAR podrá sugerir una biografía profesional basada en tu historia.",
      },
      {
        label: "Próximo paso IA",
        text: "Con esta información construiremos el núcleo de tu perfil artesanal.",
      },
    ],
  ...
  2: {
    cards: [
      {
        label: "Lectura de aprendizaje",
        text: "TELAR analizará cómo comenzó tu aprendizaje y las señales de tradición en tu historia.",
      },
      {
        label: "Señales culturales",
        text: "Al completar el significado del oficio, detectaremos los valores de tu marca personal.",
      },
      {
        label: "Territorio detectado",
        text: "Con país, departamento y municipio organizaremos el origen territorial de tu perfil.",
      },
    ],
  ...
  4: {
    cards: [
      ...
      {
        label: "Materiales clave",
        text: "Los materiales conectarán tu perfil con productos, procesos y búsquedas del marketplace.",
      },
      {
  ...
  useEffect(() => {
    if (shopAny?.artisanProfile) {
      ...
      setData({
        ...
        learnedFromDetail:
          profile.learnedFromDetail || (telarData.story?.value as string) || "",
            ...
            .then((identity) => {
              if (identity?.techniquePrimaryId)
                setData((prev) => ({
                  ...prev,
                  primaryTechniqueId:
                    prev.primaryTechniqueId ?? identity.techniquePrimaryId!,
                }));
            })
            .catch(() => {});
        }
      ...
      setData((prev) => ({
        ...
        learnedFromDetail:
          prev.learnedFromDetail || (telarData.story?.value as string) || "",
        department: prev.department || (shopAny?.department as string) || "",
        municipality:
          prev.municipality || (shopAny?.municipality as string) || "",
  ...
  const saveDraft = async (quiet = false) => {
    ...
    try {
      await updateArtisanShop(shop.id, {
        ...
        artisanProfileCompleted: isProfileCompleted,
      });
      if (!quiet)
        toast({
  ...
  if (isLast) {
    return (
      <ArtisanStepShell
        ...
        submitLabel={
          isProfileCompleted ? "Guardar cambios" : "Finalizar y publicar"
        }
      >
        <Step7Preview
          data={data}
