...
const WeightField: React.FC<WeightFieldProps> = ({
  ...
  <div className={className}>
    <div className="flex items-center justify-between mb-1.5">
      ...
      <div
        className="flex rounded-md overflow-hidden border border-[#e2d5cf]/40"
...
export const Step4PriceLogistics: React.FC<Props> = ({
  state,
  update,
  onNext,
  onBack,
  ...
  const [weightUnit, setWeightUnit] = useState<WeightUnit>(() =>
    suggestWeightUnit(state.materials ?? []),
  );
  const [pkgWeightUnit, setPkgWeightUnit] = useState<WeightUnit>(() =>
    suggestWeightUnit(state.materials ?? []),
  );
  ...
  const handleAcceptVariantSuggestion = useCallback(() => {
    ...
    for (
      let i = axes.length - 1;
      i >= 0 && combosOf() > MAX_VARIANTS_PER_PRODUCT;
      i--
    ) {
      while (
        axisValues[axes[i]].length > 1 &&
        combosOf() > MAX_VARIANTS_PER_PRODUCT
      ) {
  ...
  const handleNext = () => {
    ...
    const payload: Step2ConfirmRequest = {
      ...
      shortDescription: fm?.shortDescription
        ? {
            source: fm.shortDescription.source,
            originalAiValue: fm.shortDescription.originalAiValue ?? "",
            timestamp:
              fm.shortDescription.timestamp ?? new Date().toISOString(),
          }
        : defaultField(),
      artisanalHistory: fm?.artisanalHistory
        ? {
            source: fm.artisanalHistory.source,
            originalAiValue: fm.artisanalHistory.originalAiValue ?? "",
            timestamp:
              fm.artisanalHistory.timestamp ?? new Date().toISOString(),
          }
        : defaultField(),
      careNotes: fm?.careNotes
        ? {
            source: fm.careNotes.source,
            originalAiValue: fm.careNotes.originalAiValue ?? "",
            timestamp: fm.careNotes.timestamp ?? new Date().toISOString(),
          }
        : defaultField(),
      elaborationTime: fm?.elaborationTime
        ? {
            source: fm.elaborationTime.source,
            originalAiValue: fm.elaborationTime.originalAiValue ?? "",
            timestamp: fm.elaborationTime.timestamp ?? new Date().toISOString(),
          }
        : defaultField(),
      monthlyCapacity: fm?.monthlyCapacity
        ? {
            source: fm.monthlyCapacity.source,
            originalAiValue: fm.monthlyCapacity.originalAiValue ?? "",
            timestamp: fm.monthlyCapacity.timestamp ?? new Date().toISOString(),
          }
        : defaultField(),
      processDescription: fm?.processDescription
        ? {
            source: fm.processDescription.source,
            originalAiValue: fm.processDescription.originalAiValue ?? "",
            timestamp:
              fm.processDescription.timestamp ?? new Date().toISOString(),
          }
        : defaultField(),
      price: fm?.price
        ? {
            source: fm.price.source,
            originalAiValue: fm.price.originalAiValue ?? "",
            timestamp: fm.price.timestamp ?? new Date().toISOString(),
          }
        : defaultField(),
      weightKg: fm?.weightKg
        ? {
            source: fm.weightKg.source,
            originalAiValue: fm.weightKg.originalAiValue ?? "",
            timestamp: fm.weightKg.timestamp ?? new Date().toISOString(),
          }
        : defaultField(),
      ...(fm?.variants && {
        variants: {
  ...
  useEffect(() => {
    setNode(
      <div
        ...
        style={{ background: "#151b2d", borderRadius: 16 }}
      >
        <div className="flex items-center gap-2 border-b border-white/10 pb-4">
          ...
          <h3 className="font-['Manrope'] text-[10px] font-[800] tracking-widest uppercase text-white">
            Oráculo
          </h3>
        </div>
              ...
              <>
                {pricing.suggested_price.range && (
                  <p className="text-[10px] text-white/40 mt-1">
                    Rango: $
                    {pricing.suggested_price.range.min.toLocaleString("es-CO")}{" "}
                    – $
                    {pricing.suggested_price.range.max.toLocaleString("es-CO")}
                  </p>
                )}
        ...
        {pricing?.packaging && (
          ...
          >
            <div className="flex items-center justify-between mb-1">
              <p className="text-[9px] font-[800] text-white/40 uppercase tracking-widest">
                Empaque
              </p>
              <span className="inline-flex items-center px-1.5 py-0.5 rounded-full text-[8px] font-[800] uppercase tracking-widest bg-[#ec6d13]/20 text-[#ec6d13]">
                IA
              </span>
            </div>
            <p className="text-[13px] text-white/80 font-[600]">
              {pricing.packaging.label}
            </p>
        ...
        {pricing?.dimensions_guidance && (
          ...
          >
            ...
            <p className="text-[11px] text-white/60 leading-snug whitespace-pre-line">
              {pricing.dimensions_guidance.content}
            </p>
          </div>
        )}
        ...
        {!pricing && (
          <div className="flex flex-col items-center gap-2 py-4">
            ...
            <p className="text-[11px] text-white/30 text-center">
              Completa los pasos anteriores para recibir sugerencias del
              Oráculo.
            </p>
          </div>
        )}
        ...
        <div className="pt-3 border-t border-white/10">
          <p className="text-[9px] font-[800] text-white/40 uppercase tracking-widest mb-2">
      ...
      <main className="max-w-[1200px] mx-auto px-6 md:px-10 pt-4 pb-10 md:pt-6 md:pb-10">
        <div className="grid grid-cols-12 gap-6 items-start">
          ...
          <aside className="hidden lg:block lg:col-span-3 sticky top-8">
            <div
              className="p-5 text-white rounded-2xl flex flex-col gap-4"
            ...
            >
              ...
              {pricing?.suggested_price && (
                <Step4SuggestionCard
                  ...
                  onReject={handleRejectSuggestion}
                  isAccepted={
                    state.fieldMetadata?.price?.source === "ia_accepted"
                  }
                  isRejected={state.fieldMetadata?.price?.source === "manual"}
                  extra={
                    <>
                      {pricing.suggested_price.range && (
                        <p className="text-[10px] text-white/40 mt-1">
                          Rango: $
                          {pricing.suggested_price.range.min.toLocaleString(
                            "es-CO",
                          )}{" "}
                          – $
                          {pricing.suggested_price.range.max.toLocaleString(
                            "es-CO",
                          )}
                        </p>
                      )}
              ...
              {pricing?.estimated_weight && (
                <Step4SuggestionCard
                  ...
                  onReject={handleRejectSuggestion}
                  isAccepted={
                    state.fieldMetadata?.weightKg?.source === "ia_accepted"
                  }
                  isRejected={
                    state.fieldMetadata?.weightKg?.source === "manual"
                  }
                />
              )}
          ...
          <div className="col-span-12 lg:col-span-9 space-y-5 pb-32">
            ...
            <section className="p-6 rounded-2xl" style={cardStyle}>
              <div className="flex items-center gap-3 mb-2">
                <span className="material-symbols-outlined text-[#ec6d13] text-xl">
                  payments
                </span>
              ...
              <p className="text-[11px] text-[#54433e]/60 mb-4">
                Define tu precio base. TELAR suma un cargo al comprador y
                descuenta comisiones al vendedor.
              </p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                {state.price && state.price > 0 && (
                  <div className="flex flex-col gap-2">
                    {/* What buyer pays */}
                    <div
                      className="p-3 rounded-lg border border-[#e2d5cf]/30"
                      style={{ background: "rgba(247,244,239,0.4)" }}
                    >
                      <p className="text-[9px] font-[800] text-[#54433e]/40 uppercase tracking-widest mb-1.5">
                        El comprador paga
                      </p>
                      <div className="flex justify-between text-[11px] mb-1">
                        <span className="text-[#54433e]/60">Tu precio</span>
                        <span className="text-[#54433e]/80 font-[600]">
                          ${formatCOP(state.price)}
                        </span>
                      </div>
                      <div className="flex justify-between text-[11px] mb-1">
                        <span className="text-[#54433e]/60">
                          Cargo de servicio TELAR (+5%)
                        </span>
                        <span className="text-[#54433e]/80 font-[600]">
                          + ${formatCOP(Math.round(state.price * 0.05))}
                        </span>
                      </div>
                      <div className="flex justify-between text-[12px] pt-1.5 border-t border-[#e2d5cf]/30 mt-1">
                        <span className="font-[700] text-[#151b2d]">
                          Total al comprador
                        </span>
                        <span className="font-[800] text-[#151b2d]">
                          ${formatCOP(Math.round(state.price * 1.05))} COP
                        </span>
                      </div>
                    </div>
                    ...
                    >
                      ...
                      <div className="flex justify-between text-[11px] mb-1">
                        ...
                        <span className="text-[#54433e]/80 font-[600]">
                          − ${formatCOP(Math.round(state.price * 0.1))}
                        </span>
                      </div>
                      <div className="flex justify-between text-[11px] mb-1">
                        <span className="text-[#54433e]/60">
                        ...
                        <span className="text-[#54433e]/80 font-[600]">
                          − ${formatCOP(Math.round(state.price * 0.035))}
                        </span>
                      </div>
                      <div className="flex justify-between text-[12px] pt-1.5 border-t border-[#e2d5cf]/30 mt-1">
                        <span className="font-[700] text-[#151b2d]">
                        ...
                        <span className="font-[800] text-[#ec6d13]">
                          ${formatCOP(Math.round(state.price * 0.865))} COP
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              <div className="mt-4 pt-4 border-t border-[#e2d5cf]/30">
                ...
                <p className="flex items-center gap-1.5 text-[11px] text-[#54433e]/40 font-[500]">
                  ...
                  TELAR genera el SKU automáticamente al guardar, usando
                  categoría, territorio y técnica de la pieza.
                </p>
              </div>
            </section>
            ...
            <section className="p-6 rounded-2xl" style={cardStyle}>
              <div className="flex items-center gap-3 mb-2">
                <span className="material-symbols-outlined text-[#54433e]/40 text-xl">
                  storefront
                </span>
                <label className="font-['Manrope'] text-[10px] font-[800] text-[#151b2d] uppercase tracking-widest">
                  Stock y entrega
              ...
              {state.availabilityType && (
                <p className="text-[11px] text-[#54433e]/60 mb-4">
                  Se vende como{" "}
                  <span className="font-[800] text-[#151b2d]">
                    {AVAILABILITY_LABELS[state.availabilityType]}
                  </span>{" "}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      const updates: Partial<NewWizardState> = {
                        inventory: val,
                      };
                      if (
                        state.minimumStockAlert !== undefined &&
                        state.minimumStockAlert > val
                      ) {
                    ...
                    disabled={
                      state.productionType === "unica" || hasActiveVariants
                    }
                  ...
                  {hasActiveVariants ? (
                    <p className="text-[10px] text-[#ec6d13]/80 mt-1.5 font-[600]">
                      Suma automática del stock de tus {activeVariantsCount}{" "}
                      variantes.
                    </p>
                  ) : (
                    stockHint && (
                ...
                <div>
                  ...
                  {state.inventory !== undefined &&
                    state.minimumStockAlert !== undefined &&
                    state.minimumStockAlert > state.inventory && (
                      <p className="text-[10px] text-red-500 mt-1.5 font-[600]">
                        La alerta no puede superar el stock disponible.
                      </p>
                    )}
                </div>
              </div>
              ...
              {state.productionType === "bajo_pedido" && (
                <div className="mt-4 pt-4 border-t border-[#e2d5cf]/30">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      update({ elaborationTime: e.target.value })
                    }
            ...
            <section
              id="wizard-field-packageDims"
              className="p-6 rounded-2xl"
              style={{ ...cardStyle, borderLeft: "3px solid #ec6d13" }}
            >
              <div className="flex items-center gap-3 mb-2">
                <span className="material-symbols-outlined text-[#ec6d13] text-xl">
                  local_shipping
              ...
              <p className="text-[11px] text-[#54433e]/60 mb-4">
                Medidas logísticas con empaque incluido. Se usan para calcular
                el costo de envío.
              </p>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
              ...
              <div className="mt-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[10px] text-[#54433e]/40">
                    Para piezas frágiles o delicadas
                  </p>
                </div>
                <div
                  className="flex p-1 rounded-lg border border-[#e2d5cf]/40"
                  style={{ background: "rgba(247,244,239,0.3)" }}
                >
                  {["Sí", "No"].map((opt) => (
                    <button
                      ...
                      style={{
                        background:
                          (state.specialHandling === true && opt === "Sí") ||
                          (state.specialHandling === false && opt === "No")
                            ? "#ec6d13"
                            : "transparent",
                        color:
                          (state.specialHandling === true && opt === "Sí") ||
              ...
              {state.specialHandling === true && (
                <div className="mt-3">
                  <textarea
                    ...
                    onChange={(e) =>
                      update({ shippingRestrictions: e.target.value })
                    }
