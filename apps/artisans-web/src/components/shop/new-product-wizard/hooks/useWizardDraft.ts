...
export const mapNewStateToDto = (
  ...
  if (
    ...
    dto.artisanalIdentity = {
      ...
      ...(isUUID(state.primaryTechniqueId) && {
        primaryTechniqueId: state.primaryTechniqueId,
      }),
      ...(isUUID(state.secondaryTechniqueId) && {
        secondaryTechniqueId: state.secondaryTechniqueId,
      }),
      pieceType: state.purpose,
      // `style` legacy = primer estilo; `styles` lleva la selección completa
      style:
        firstStyle === "tradicional"
          ? "tradicional"
          : firstStyle === "contemporaneo"
            ? "contemporaneo"
            : firstStyle === "fusion"
              ? "fusion"
              : undefined,
  ...
  if (availabilityType) {
    dto.production = {
      ...
      processEvidenceUrls:
        (state.processEvidenceUrls ?? []).filter(
          ...
          : undefined,
      tools:
        (state.tools ?? []).filter((t) => t.trim()).length > 0
  ...
  if (state.hasVariants && (state.variants?.length ?? 0) > 0) {
    dto.variants = state
      .variants!.filter((v) => (v.price ?? state.price ?? 0) > 0)
      .map((v) => ({
        ...(v.id && { id: v.id }),
...
export const useWizardDraft = (
  ...
  const saveDraft = useCallback(
    async (silent = false) => {
      if (!shopId || !state.name.trim() || !state.shortDescription.trim())
        return;
      setIsSavingDraft(true);
      try {
        // Extraer URLs ya subidas desde state.images (las que son string, no File)
        const uploadedUrls = (state.images ?? []).filter(
