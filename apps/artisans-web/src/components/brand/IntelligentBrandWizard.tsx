...
import {
  ...
  generateClaim,
  diagnoseBrandIdentity,
} from "@/services/brandAiAssistant.actions";
...
export const IntelligentBrandWizard: React.FC = () => {
  ...
  const { context, updateProfile, updateContext, refreshData } =
    useUnifiedUserData();
      ...
      try {
        ...
        if (!effectiveDescription && !effectiveBrandName) {
          toast({
            ...
            description:
              "Necesitamos al menos el nombre de tu negocio para continuar.",
            variant: "destructive",
          });
    ...
    try {
      ...
      try {
        ...
        toast({
          title: "Advertencia",
          description:
            "No se pudieron extraer colores automáticamente. Puedes continuar.",
          variant: "default",
        });
        setColorSystem({
      ...
      EventBus.publish("brand.logo.uploaded", {
        logoUrl: publicUrl,
  ...
  const generateSecondaryPalette = async (primaryColors: string[]) => {
    ...
    try {
      const { secondary_colors, reasoning } =
        await generateColorPalette(primaryColors);

      if (!secondary_colors?.length)
        throw new Error("No secondary colors returned");
      ...
      toast({
        title: "Advertencia",
        description:
          "Se usaron colores predeterminados para la paleta secundaria.",
        variant: "default",
      });
  ...
  const runBrandDiagnosis = async (
    finalPerceptionData: typeof perceptionData,
  ) => {
    if (!user) return;
    ...
    try {
      ...
      const missions = generateBrandMissions(
        diagnosisResult,
  ...
  const handleSaveBrand = async () => {
    ...
    try {
      ...
      toast({
        ...
        description:
          "Tu identidad visual está completa y aplicada a tu tienda.",
      });
    ...
    return (
      <Card className="max-w-2xl mx-auto">
        <CardHeader>
          ...
          <CardDescription>
            Tu logo es la cara de tu marca. La IA extraerá automáticamente los
            colores dominantes.
          </CardDescription>
        </CardHeader>
              ...
              {logoUrl ? (
                ...
                <div className="py-12">
                  ...
                  <p className="text-muted-foreground">
                    Click para seleccionar tu logo
                  </p>
                </div>
              )}
            </Label>
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          <Loader2 className="w-12 h-12 animate-spin mx-auto mb-4 text-primary" />
          <p className="text-lg font-semibold mb-2">
            Generando paleta de colores secundaria...
          </p>
          <p className="text-muted-foreground">
            La IA está creando colores complementarios para tu marca
          </p>
        </div>
      </div>
    );
  ...
  if (
    ...
    const questionNumber =
      step === "perception-1" ? 1 : step === "perception-2" ? 2 : 3;
    return (
      <BrandPerceptionStep
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">
            La IA está evaluando logo, colores, mensaje y coherencia
          </p>
        </div>
      </div>
    );
    ...
    return (
      <BrandHub
        ...
        onElementUpdated={async () => {
          ...
          const brandEval = (contextData?.conversationInsights as any)
            ?.brand_evaluation;
        ...
        <BrandDashboardView
          ...
          onElementUpdated={async (element, newDiagnosis) => {
            ...
            if (newDiagnosis.average_score < (diagnosis?.average_score || 0)) {
              const newMissions = generateBrandMissions(
                newDiagnosis,
                brandContext.brandName,
              );
            ...
            const brandEval = (freshContext?.conversationInsights as any)
              ?.brand_evaluation;
        ...
        <CardContent className="space-y-6">
          ...
          <div className="pt-4 border-t space-y-4">
            <Label className="font-semibold">
              ¿Ya tienes un claim para tu marca?
            </Label>
            <p className="text-sm text-muted-foreground">
              Un claim es una frase corta que define la esencia de tu marca (ej:
              "Nike - Just Do It")
            </p>
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">
            La IA está creando opciones para tu marca
          </p>
        </div>
      </div>
    );
    ...
    return (
      <Card className="max-w-2xl mx-auto">
        <CardHeader>
          ...
          <CardDescription>
            La IA generó 3 opciones. Selecciona la que mejor representa tu
            marca.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="mt-8 p-6 bg-primary/5 border border-primary/20 rounded-lg">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Ahora que has definido tu identidad de marca, configura el Hero
              Slider de tu tienda para que refleje tu nueva imagen visual.
            </p>
            <Button
