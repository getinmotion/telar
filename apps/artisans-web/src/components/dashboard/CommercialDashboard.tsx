        ...
        <header className="sticky top-0 z-30">
          <div className="px-4 md:px-12 py-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              {shop?.logoUrl && (
                <img src={shop.logoUrl} alt={shopName} className="hidden md:block h-9 w-9 rounded-full object-contain"
            ...
            <div className="flex md:hidden items-center gap-2">
              {agreementName && (
                <button onClick={() => setShowBadgeInfo(v => !v)} className="w-8 h-8 flex items-center justify-center rounded-full"
                  style={{ background: 'rgba(21,27,45,0.05)', border: '1px solid rgba(21,27,45,0.08)' }}>
              ...
              {isMarketplaceLive ? (
                ...
                <button onClick={() => setShowBadgeInfo(v => !v)}
          ...
          {/* Mobile expandable badge info */}
          {showBadgeInfo && agreementName && (
            <div className="px-4 pb-2">
              <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full"
                style={{ background: 'rgba(21,27,45,0.05)', border: '1px solid rgba(21,27,45,0.08)', display: 'inline-flex' }}>
                <span className="material-symbols-outlined" style={{ fontSize: 14, color: 'rgba(84,67,62,0.6)' }}>handshake</span>
                <span style={{ fontFamily: SANS, fontSize: 11, fontWeight: 700, color: 'rgba(84,67,62,0.7)', whiteSpace: 'nowrap' }}>{agreementName}</span>
              </div>
            </div>
          ...
          <div className="max-w-[1300px] mx-auto pt-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 md:gap-6 mb-8">
              <div className="flex items-center gap-3 min-w-0">
                {shop?.logoUrl ? (
                  <img src={shop.logoUrl} alt={shopName} className="w-10 h-10 rounded-xl object-contain shrink-0"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              <div className="lg:col-span-8 space-y-8">
                ...
                {isActivated && !isMarketplaceLive ? (
                  ...
                  >
                    <div className="flex-1 relative z-10 w-full text-left">
                      ...
                      <div className="flex gap-3 flex-wrap items-center justify-start">
                        <button onClick={reviewSl.action}
                      ...
                      <div className="flex items-center justify-start gap-2 mt-6">
                        {REVIEW_SLIDES.map((_, i) => (
                          <button key={i} onClick={() => setReviewSlide(i)}
                            style={{ width: i === reviewSlide ? 20 : 6, height: 6, borderRadius: 3, background: i === reviewSlide ? '#3b82f6' : 'rgba(21,27,45,0.12)', border: 'none', cursor: 'pointer', padding: 0, transition: 'width 0.3s ease, background 0.2s' }} />
                        ))}
                      </div>
                    </div>
                    <div className="hidden md:block shrink-0 relative w-44 h-44">
                      ...
                      <div className="absolute inset-4 rounded-2xl -rotate-3 flex items-center justify-center" style={{ ...glassPrimary, borderRadius: 20 }}>
                        <span className="material-symbols-outlined" style={{ fontSize: 64, color: 'rgba(59,130,246,0.25)' }}>{reviewSl.icon}</span>
                      </div>
                  ...
                  >
                    ...
                    <div className="shrink-0 relative w-44 h-44">
                      ...
                      <div className="absolute inset-4 rounded-2xl -rotate-3 flex items-center justify-center" style={{ ...glassPrimary, borderRadius: 20 }}>
                        <span className="material-symbols-outlined" style={{ fontSize: 64, color: `${nextCard.accentColor}30` }}>{nextCard.icon}</span>
                      </div>
                      <div className="absolute -top-3 -right-3 w-12 h-12 rounded-full flex items-center justify-center"
                        style={{ background: 'white', border: '1px solid rgba(255,255,255,0.8)', boxShadow: '0 4px 12px rgba(21,27,45,0.08)' }}>
                ...
                <div style={{ ...glassPrimary, borderRadius: 32 }} className="p-6 md:p-10">
                    ...
                    <div className="flex gap-2 flex-wrap items-center">
                      {isMarketplaceLive ? (
                        <>
                          ...
                          <button onClick={() => setShowBioModal(true)} className="flex items-center gap-1.5 px-4 py-1.5 rounded-full transition-all hover:opacity-90"
                    ...
                    {checklistItems.map((item) => (
                      <button key={item.label} onClick={() => navigate(item.route)}
                        ...
                        <span className="material-symbols-outlined shrink-0"
                          style={{ fontSize: 20, color: item.done ? (isMarketplaceLive ? '#166534' : isActivated ? '#3b82f6' : '#ec6d13') : 'rgba(21,27,45,0.15)', fontVariationSettings: item.done ? "'FILL' 1" : "'FILL' 0" }}>
                          {item.done ? 'check_circle' : 'radio_button_unchecked'}
                        </span>
                        <span style={{ fontFamily: SANS, fontSize: 14, fontWeight: 700, color: item.done ? '#151b2d' : 'rgba(21,27,45,0.35)' }}>
                          {item.label}
                          {item.done && (
                            <span style={{ fontFamily: SANS, fontSize: 10, fontWeight: 600, color: '#ec6d13', marginLeft: 8, opacity: 0 }} className="group-hover:opacity-100 transition-opacity">Editar</span>
                          )}
                          {!item.done && item.required && (
                            <span style={{ fontFamily: SANS, fontSize: 8, fontWeight: 900, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#ef4444', marginLeft: 6 }}>Obligatorio</span>
                          )}
                          {!item.done && !item.required && (
                            <span style={{ fontFamily: SANS, fontSize: 8, fontWeight: 900, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(21,27,45,0.25)', marginLeft: 6 }}>Recomendado</span>
                          )}
                        </span>
                      </button>
                    ))}
                  ...
                  <div className="pt-6 flex flex-col gap-4" style={{ borderTop: '1px solid rgba(21,27,45,0.04)', marginTop: 24 }}>
                    {!isActivated && (
                      <button
                        onClick={() => requiredPending.length === 0 ? setShowPublishDialog(true) : navigate(requiredPending[0].route)}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <button onClick={() => shopSlug && window.open(buildMarketplaceStoreUrl(shopSlug), '_blank')} disabled={!shopSlug}
                        ...
                        <div className="absolute top-3 right-3 flex items-center gap-1 px-2 py-0.5 rounded-full"
                          style={{ background: isMarketplaceLive ? 'rgba(22,101,52,0.9)' : isActivated ? 'rgba(59,130,246,0.9)' : 'rgba(255,255,255,0.12)' }}>
                        ...
                        <div>
                          ...
                          <p style={{ fontFamily: SANS, fontSize: 10, fontWeight: 500, color: 'rgba(255,255,255,0.45)', marginTop: 1 }}>
                            {isMarketplaceLive ? 'Visible en el marketplace' : isActivated ? 'En revisión editorial' : 'Así se verá tu tienda'}
                          </p>
                        </div>
                      </button>
                    </div>
                ...
                <div className="md:hidden rounded-2xl p-5 flex items-center justify-between gap-4" style={{ ...glassPrimary, borderRadius: 20 }}>
                  <div>
                    <p style={{ fontFamily: SERIF, fontSize: 18, fontWeight: 700, color: '#151b2d', lineHeight: 1.2 }}>Mis productos</p>
                  ...
                  <button onClick={() => navigate('/productos/subir')} className="flex items-center gap-1.5 px-4 py-2.5 rounded-full shrink-0"
                ...
                <div className="hidden md:block">
                  <ProductsTable products={products} loadingProducts={loadingProducts} isActivated={isActivated} />
                </div>
              </div>
              ...
              <aside className="lg:col-span-4 space-y-6">
                ...
                <div className="md:hidden rounded-2xl p-5 flex items-center justify-between gap-4" style={{ ...glassPrimary, borderRadius: 20 }}>
                  <div>
                    <p style={{ fontFamily: SERIF, fontSize: 18, fontWeight: 700, color: '#151b2d', lineHeight: 1.2 }}>Mis ventas</p>
                    <p style={{ fontFamily: SANS, fontSize: 12, fontWeight: 500, color: 'rgba(84,67,62,0.6)', marginTop: 3 }}>
                      {salesStats.totalRevenue > 0 ? formatCurrency(salesStats.totalRevenue) : 'Sin ingresos aún'}
                    </p>
                    <button onClick={() => navigate('/mi-tienda/ventas')}
                      style={{ fontFamily: SANS, fontSize: 11, fontWeight: 700, color: 'rgba(84,67,62,0.5)', marginTop: 6, background: 'none', border: 'none', padding: 0, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
                      Ver pedidos
                      <span className="material-symbols-outlined" style={{ fontSize: 14 }}>east</span>
                    </button>
                  </div>
                  <button onClick={() => navigate('/mi-tienda/ventas')} className="flex items-center gap-1.5 px-4 py-2.5 rounded-full shrink-0"
                  ...
                  <div className="relative z-10">
                    <div className="flex justify-between items-start mb-5">
                      <div>
                        ...
                        <h3 style={{ fontFamily: SERIF, fontSize: 18, fontWeight: 700, color: '#151b2d' }}>{maturityLabel}</h3>
                      </div>
                    ...
                    <button onClick={reviewSl.action} className="flex items-center gap-2 px-5 py-2.5 rounded-full transition-opacity hover:opacity-80"
                      style={{ background: '#3b82f6', color: 'white', fontFamily: SANS, fontSize: 13, fontWeight: 700, border: 'none', cursor: 'pointer' }}>
                      {reviewSl.cta}
                      <span className="material-symbols-outlined text-[16px]">east</span>
                    </button>
            ...
            {/* Footer */}
            <footer className="flex flex-col md:flex-row items-center md:justify-between gap-1 md:gap-0 py-6 mt-8 text-center md:text-left"
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4"
          ...
          <div className="w-full max-w-md rounded-3xl p-8"
            ...
            <div className="flex gap-3">
              <button onClick={handleConfirmPublish} disabled={publishLoading}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4"
          ...
          <div className="w-full overflow-hidden"
            ...
            <div className="px-8 pt-7 pb-5 flex items-center justify-between" style={{ borderBottom: '1px solid rgba(21,27,45,0.06)' }}>
              ...
              <button onClick={() => setShowBioModal(false)} className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-black/5 transition-all">
            ...
            <div className="p-8 space-y-6 max-h-[75vh] overflow-y-auto">
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2 px-4 py-3 rounded-xl" style={{ background: 'rgba(21,27,45,0.03)', border: '1px solid rgba(21,27,45,0.06)' }}>
                  <span className="flex-1 truncate" style={{ fontFamily: SANS, fontSize: 12, fontWeight: 500, color: 'rgba(84,67,62,0.6)' }}>{bioUrl}</span>
                  <button onClick={handleCopyBioLink} className="shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full transition-all hover:opacity-90"
                    style={{ background: bioCopied ? 'rgba(22,101,52,0.1)' : '#ec6d13', color: bioCopied ? '#166534' : 'white', fontFamily: SANS, fontSize: 11, fontWeight: 800 }}>
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between px-4 py-3 rounded-xl cursor-pointer" style={{ ...glassSecondary }}
                    ...
                    <div className="w-10 h-6 rounded-full transition-all relative" style={{ background: bioConfig.showShopLink ? '#ec6d13' : 'rgba(21,27,45,0.12)' }}>
                      <div className="absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-all" style={{ left: bioConfig.showShopLink ? 22 : 2 }} />
                    </div>
                  </div>
                  <div className="flex items-center justify-between px-4 py-3 rounded-xl cursor-pointer" style={{ ...glassSecondary }}
                    ...
                    <div className="w-10 h-6 rounded-full transition-all relative" style={{ background: bioConfig.showProfileLink ? '#ec6d13' : 'rgba(21,27,45,0.12)' }}>
                      <div className="absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-all" style={{ left: bioConfig.showProfileLink ? 22 : 2 }} />
                    </div>
                  </div>
                </div>
              </div>
              <div className="space-y-2">
                ...
                <select value={bioConfig.featuredProductId ?? ''} onChange={(e) => setBioConfig(c => ({ ...c, featuredProductId: e.target.value || null }))}
                  className="w-full px-4 py-3 rounded-xl appearance-none"
                  style={{ fontFamily: SANS, fontSize: 13, fontWeight: 600, color: '#151b2d', background: 'rgba(255,255,255,0.82)', border: '1px solid rgba(21,27,45,0.1)', outline: 'none' }}>
            ...
            <div className="px-8 pb-7 pt-2 flex gap-3" style={{ borderTop: '1px solid rgba(21,27,45,0.06)' }}>
              <button onClick={handleSaveBioConfig} disabled={bioSaving}
                className="flex-1 flex items-center justify-center gap-2 py-3 rounded-full transition-all hover:opacity-90"
                style={{ background: '#ec6d13', color: 'white', fontFamily: SANS, fontSize: 13, fontWeight: 700, boxShadow: '0 4px 12px rgba(236,109,19,0.3)', opacity: bioSaving ? 0.7 : 1 }}>
                {bioSaving ? (
