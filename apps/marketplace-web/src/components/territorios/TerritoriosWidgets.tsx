/**
 * TerritoriosWidgets — bloque interactivo del mapa de territorios.
 *
 * Es atómico porque mapa + spotlight + lista índice comparten estado
 * (mapRef + selectedTerritory + hoveredTerritory) y la lista hace
 * fly-to en el mapa al hover. El curador puede poner secciones
 * editoriales antes o después de este widget desde el admin, pero no
 * dentro de él.
 */
...
const TERRITORIES: TerritoryPoint[] = [
  { slug: 'la-guajira',  name: 'La Guajira',          subtitle: 'Tejeduría Wayúu · Desierto y Mar',     lat: 11.55, lng: -72.9,  color: '#ec6d13', size: 16, techniques: 'Tejeduría Wayúu',                       region: 'Caribe'    },
  { slug: 'san-jacinto', name: 'San Jacinto',         subtitle: 'Hamacas y Telar · Montes de María',    lat: 9.83,  lng: -75.12, color: '#ec6d13', size: 22, techniques: 'Telar vertical, Macramé',                 region: 'Caribe'    },
  { slug: 'boyaca',      name: 'Boyacá',              subtitle: 'Tradición Textil · Altiplano',          lat: 5.53,  lng: -73.36, color: '#ec6d13', size: 18, techniques: 'Hilado manual, Ruanas',                   region: 'Andina'    },
  { slug: 'narino',      name: 'Nariño',              subtitle: 'Barniz de Pasto · Andes Volcánicos',    lat: 1.28,  lng: -77.35, color: '#0098f2', size: 18, techniques: 'Barniz de Pasto, Paja Toquilla',          region: 'Pacífico'  },
  { slug: 'la-chamba',   name: 'La Chamba',           subtitle: 'Barro Negro · Río Magdalena',           lat: 3.68,  lng: -75.02, color: '#584237', size: 14, techniques: 'Cerámica de barro negro',                 region: 'Andina'    },
  { slug: 'putumayo',    name: 'Putumayo',            subtitle: 'Cestería y Semillas · Selva Amazónica', lat: 0.77,  lng: -76.64, color: '#0098f2', size: 14, techniques: 'Cestería, Semillas',                      region: 'Amazonía'  },
  { slug: 'cauca',       name: 'Cauca',               subtitle: 'Seda y Tintes de Paz',                  lat: 2.44,  lng: -76.61, color: '#ec6d13', size: 20, techniques: 'Sericultura, Tintes Naturales, Telar',    region: 'Pacífico'  },
];
...
/**
 * Widget atómico: mapa interactivo + tiendas sin ubicar + spotlight +
 * lista índice. Payload opcional para customizar el caption del mapa.
 */
export function TerritoriosMapBlockWidget({
  ...
  const flyToTerritory = useCallback((t: TerritoryPoint) => {
    ...
    mapRef.current?.flyTo({ center: [t.lng, t.lat], zoom: 8, duration: 1400 });
  }, []);

  const resetView = useCallback(() => {
    setSelectedTerritory(null);
    mapRef.current?.flyTo({ center: [-73.5, 4.5], zoom: 5.2, duration: 1200 });
  }, []);
    ...
    <>
      ...
      <section className="w-full py-24 relative overflow-hidden" style={{ backgroundColor: '#f5f3ee' }}>
        <div className="max-w-[1400px] mx-auto px-8 relative z-10 flex flex-col items-center">
          <div
            ...
            style={{ aspectRatio: '16/9', boxShadow: '0 20px 40px rgba(27, 28, 25, 0.06)' }}
          >
            <Map
              ref={mapRef}
              initialViewState={{ longitude: -73.5, latitude: 4.5, zoom: 5.2 }}
                ...
                >
                  ...
                  >
                    <div
                      ...
                      style={{ backgroundColor: t.color, width: t.size, height: t.size }}
                    />
                    <div
                      className="rounded-full border-2 transition-transform duration-300"
                      style={{
                        width: t.size, height: t.size,
                        backgroundColor: t.color, borderColor: '#fff',
                        transform: hoveredTerritory === t.slug || selectedTerritory?.slug === t.slug ? 'scale(1.4)' : 'scale(1)',
                      }}
                    />
      ...
      {unlocatedShops.length > 0 && (
        <section className="max-w-[1400px] mx-auto px-8 py-12">
          <span className="text-xs tracking-widest uppercase mb-4 block font-sans font-bold" style={{ color: '#584237' }}>
            {unlocatedLabel}
          </span>
      ...
      <section className="max-w-[1400px] mx-auto px-8 py-24">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
          <div className="lg:col-span-5 order-2 lg:order-1">
            <span className="text-sm tracking-widest uppercase mb-4 block font-sans font-bold" style={{ color: '#ec6d13' }}>
              {spotlightKicker}
            </span>
            ...
            <div className="space-y-6 mb-10">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <span className="block text-[10px] uppercase tracking-widest mb-1 font-sans" style={{ color: '#584237' }}>Técnicas Clave</span>
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  <span className="block text-[10px] uppercase tracking-widest mb-1 font-sans" style={{ color: '#584237' }}>Región</span>
          ...
          <div className="lg:col-span-7 order-1 lg:order-2">
            <div
              ...
              style={{ backgroundColor: '#eae8e3', boxShadow: '0 20px 40px rgba(27, 28, 25, 0.06)' }}
            >
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-8xl font-black tracking-tighter uppercase font-serif" style={{ color: 'rgba(27,28,25,0.03)' }}>
                  {spotlight.region}
                </span>
              </div>
              <div className="absolute bottom-6 right-6 text-right" style={{ color: '#fff' }}>
                <span className="block text-[10px] uppercase tracking-[0.2em] mb-1 font-sans">Territorio</span>
            ...
            >
              <div className="flex flex-col md:flex-row justify-between items-baseline gap-4">
                <div className="flex items-baseline gap-8">
                  ...
                  <h4
                    className="text-4xl md:text-6xl font-serif group-hover:italic transition-all"
                    style={{ color: hoveredTerritory === t.slug ? '#ec6d13' : '#1b1c19' }}
                  >
                    {t.name}
                  </h4>
                </div>
