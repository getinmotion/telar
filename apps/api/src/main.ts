  ...
  const app = await NestFactory.create(AppModule, {
    cors: {
      origin: (origin, callback) => {
        ...
        // (vite preview elige puerto dinámico; dev móvil usa IP de red).
        if (
          process.env.NODE_ENV !== 'production' &&
          /^https?:\/\/(localhost|127\.0\.0\.1|192\.168\.\d{1,3}\.\d{1,3}|10\.\d{1,3}\.\d{1,3}\.\d{1,3}|172\.(1[6-9]|2\d|3[01])\.\d{1,3}\.\d{1,3})(:\d+)?$/i.test(origin)
        ) {
          return callback(null, true);
        }
